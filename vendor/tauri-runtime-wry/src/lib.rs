...
fn navigation_allows(url: &str, handler: impl FnOnce(&Url) -> bool) -> bool {
...
mod navigation_tests {
  ...
  use super::navigation_allows;

  #[test]
  fn malformed_navigation_is_denied_without_calling_the_handler() {
    let called = Cell::new(false);

    assert!(!navigation_allows("https://[::1", |_| {
      called.set(true);
      true
    }));
    assert!(!called.get());
  ...
  #[test]
  fn parsed_navigation_uses_the_handler_decision() {
    assert!(navigation_allows("https://example.com", |url| url.scheme() == "https"));
    assert!(!navigation_allows("https://example.com", |_| false));
  }
}
...
"#,
  ...
  if let Some(navigation_handler) = pending.navigation_handler {
    webview_builder = webview_builder
      .with_navigation_handler(move |url| navigation_allows(&url, |url| navigation_handler(url)));
  }
