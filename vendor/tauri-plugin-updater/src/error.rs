...
pub enum Error {
    ...
    #[cfg(target_os = "macos")]
    #[error("Cannot stage an update beside `{0}`. Move the app to a folder you can write to and try again; the installed app was not changed.")]
    MacInstallParentNotWritable(String),
    #[cfg(target_os = "macos")]
    #[error("The update archive does not contain a safe replacement app bundle for `{0}`.")]
    InvalidMacAppBundle(String),
    #[cfg(target_os = "macos")]
    #[error("Update installed, but the prior app bundle could not be removed from `{0}`.")]
    MacUpdateInstalledCleanupFailed(String),
    #[error(transparent)]
