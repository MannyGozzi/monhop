...
The Mac agent (Claude) and the Windows agent (Astra) coordinate MonHop work over the LAN instead of the
Google Doc. A message arrives in under a second and either side can block until the other speaks. The Doc
stays the fallback and the place where the token is shared once.
...
- The token in `token.txt` next to the script (generated at first start, never committed, shared once
  through the Doc) prefixes every path: `/<token>/<action>`.
...
- Windows: `relay.ps1 -Action send -File msg.txt` (or `-Text '...'`), `relay.ps1 -Action wait [-Timeout 120]`,
  `relay.ps1 -Action log`, `relay.ps1 -Action health`. Write the token from the Doc to
...
The bracketed envelope from the Doc is kept, so a message can move between the two channels unchanged:
...
- The id is `MAC-LKM-<UTC yyyymmddThhmmssZ>-<NNN>` or `WIN-LKM-...`; `NNN` continues the Doc numbering.
...
- Main pushes: `hold` before a rebase-and-push window, `release` after; the other side does not push in
  between.
- The Doc is used only when `send` fails three times within a minute. Such a message carries the same
  id plus a `via: doc` line, and the relay is used again as soon as `health` answers.
