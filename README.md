...
Connect opens a standing, authenticated setup link on the selected physical network: either computer can press Connect first, the dialing side retries every two seconds, heartbeats detect loss, and displays refresh while connected. Choose the input computer (this computer or the other one) and arrange the seam on **either** computer, then choose **Apply on both computers**. The other computer validates the proposal against its current identities and monitor geometry and acknowledges automatically; both save the same source and seams only after that acknowledgment. Each machine retains its own network selection, permissions and protected pairing. A rejected or unacknowledged proposal is reported as not applied. Sync never starts input. Both apps must use the same protocol-version-7 build. The link closes itself after fifteen idle minutes and whenever the network changes.

The controlled check now preserves the first native startup/stop reason. Failed window validation does not consume a start ticket, and duplicate unchanged move/resize notifications do not cancel a test. Actual movement, resizing or focus loss still stops it. These fixes do not prove physical cross-machine delivery or remove the dedicated-window restriction.
...
Closing or hiding the main window keeps MonHop in the macOS menu bar or Windows notification-area tray. Its menu provides Show, Hide, Stop and Quit. The icon is neutral while input is local, green for an active native test/session, and amber while starting, stopping or needing attention. Text distinguishes a controlled test from general sharing. Hide is rejected while the controlled test window is open. Quit waits for native cleanup and in-flight local metadata writes. A failed tray installation leaves the main window visible and retains close-to-quit behavior. No login/startup service is installed.
...
## Controlled two-computer input test

Both computers must install the same current build. This is an explicit test of the input computer’s keyboard and mouse reaching MonHop’s own test window on the other computer, in either direction, not control of other applications. It does not qualify the outstanding system-shortcut or full-desktop tests.

1. **T1:** Connect both computers, choose the input computer and check both display sets. Drag the computer groups until their highlighted seam matches your intended crossing, review it, then apply it on both computers.
2. **T2:** Choose **Open test window** on both. The setup link closes first, since the test uses the same pinned port. Press **Start** on the receiving computer, then on the input computer. Keep both windows in front and avoid system shortcuts.
3. **T3:** Move across the arranged seam. Try ordinary keys, clicking, dragging and both scroll axes. The receiving window reports native arrivals to MonHop and what landed in the window, never keys or text. Marked events are consumed before AppKit/WebKit dispatch. Protocol counts are separate and do not prove native delivery.
4. **T4:** Press **Stop** or move focus away. The trial also ends after 60 seconds, network/display/permission loss or receiver failure. Closing waits for managed release cleanup. A new attempt needs a fresh inspection and one-use start approval.

The trial authenticates a distinct protocol purpose that older builds reject. Its independent native watchdog enforces focus and the deadline without the webview or async worker. macOS posts only to its own process, maps pointer positions into the test pad, rejects system shortcuts and consumes every marked event before application dispatch, counting only the current trial’s authorized arrivals. Windows injects only while its test window is in the foreground and rejects Control, Alt and Windows-key combinations, so Alt+Tab or the Start menu cannot move focus during a test. Pointer events without an AppKit window association must still lie inside the native test pad, and scrolling follows the submitted virtual cursor. The normal sharing gate remains closed. Windows source release/restore still uses native global input APIs, so this is not a sandbox for physical Windows shortcuts.
