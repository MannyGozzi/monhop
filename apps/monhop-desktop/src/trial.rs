...
mod tests {
    ...
    fn prepared_test_is_inert_and_revocation_removes_start_ticket() {
        ...
        trial
            .prepare("1".into(), layout(), "Office Windows PC".into())
            .unwrap();
        ...
        assert!(
            trial
                .prepare("2".into(), layout(), "Office Windows PC".into())
                .is_err()
        );
        trial.revoke();
    ...
    fn the_last_outcome_is_kept_across_closing_and_reopening_the_window() {
        ...
        trial
            .prepare("1".into(), layout(), " Office Windows PC \u{7}".into())
            .unwrap();
        assert!(trial.last_result().is_none());
        assert_eq!(trial.view(&sharing).peer_name, "Office Windows PC");
        {
        ...
        assert!(
            last.report()
                .contains("This computer receives input from Office Windows PC")
        );
        trial.revoke();
        {
            let mut state = lock(&trial.0);
    ...
    fn a_refused_launch_returns_the_ticket_and_keeps_start_retryable() {
        ...
        let sharing = SharingController::default();
        trial
            .prepare("7".into(), layout(), "Office Windows PC".into())
            .unwrap();
