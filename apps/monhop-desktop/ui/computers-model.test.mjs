...
test("the computer list is bounded, deduplicated, and lowercased on the way in", () => {
  const view = normalizeComputers({
    computers: [
      { fingerprint: WINDOWS.toUpperCase(), name: "Noctua Windows PC", platform: "windows" },
  ...
  assert.equal(view.items[0].name, "Noctua Windows PC");
...
test("a computer nobody is paired with is never the one in use", () => {
  const view = normalizeComputers({
    computers: [{ fingerprint: WINDOWS, name: "Noctua Windows PC", platform: "windows" }],
    active: MAC,
  });
  assert.equal(view.active, null);
  assert.equal(findComputer(view, MAC), null);
  assert.equal(findComputer(view, WINDOWS).name, "Noctua Windows PC");
...
test("each computer carries its own saved layout, or an honest empty one", () => {
  const view = normalizeComputers({
    computers: [
      { fingerprint: WINDOWS, name: "Noctua Windows PC", platform: "windows", setup: setup() },
...
test("a computer without a name falls back to its platform", () => {
  ...
  const hostile = normalizeComputers({
    computers: [{ fingerprint: WINDOWS, name: "Noctua\u202ePC", platform: "windows" }],
  });
